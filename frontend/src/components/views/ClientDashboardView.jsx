  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        setLoading(true)
        setError(null)
        const res = await api.get(`/clients/${clientId}/dashboard`)
        setData(res.data)
      } catch (err) {
  ...
  if (error || !data?.client) {
    return (
      <div className='p-8'>
        ...
        <button type='button' onClick={() => navigate('/clients')} className='mt-4 text-blue-600 text-sm font-medium'>
          Back to Clients
        </button>
      </div>
    )
    ...
    <div className='p-8 w-full'>
      <div className='flex flex-wrap items-start justify-between gap-4 mb-8'>
        <div>
          ...
          <p className='text-gray-600 text-sm mt-1'>Projects, billing, tasks, and activity for this client.</p>
        </div>
      ...
      <section className='mb-10'>
        ...
        <div className='bg-white rounded-lg shadow border border-gray-100 overflow-x-auto'>
          <table className='w-full text-sm min-w-[900px]'>
            ...
            <tbody>
              {projects.length === 0 ? (
                <tr><td colSpan={8} className='px-4 py-8 text-center text-gray-500'>No projects for this client.</td></tr>
              ) : (
                projects.map((p) => (
                  <tr key={p._id} className='border-b border-gray-100 hover:bg-gray-50'>
                    <td className='px-4 py-3 font-medium text-gray-900'>
                      <div className='flex flex-col gap-1 items-start'>
                        <button
                          type='button'
                          onClick={() => navigate(`/projects?focus=${p._id}`)}
                    ...
                    <td className='px-4 py-3 text-emerald-700 font-medium'>{fmtMoney(p.paidFromBilling)}</td>
                    <td className='px-4 py-3 text-amber-800 font-medium'>{fmtMoney(p.remainingAmount)}</td>
      ...
      <section className='mb-10'>
        ...
        <div className='bg-white rounded-lg shadow border border-gray-100 overflow-x-auto max-h-[480px] overflow-y-auto'>
          <table className='w-full text-sm'>
            ...
            <tbody>
              {tasks.length === 0 ? (
                <tr><td colSpan={7} className='px-4 py-8 text-center text-gray-500'>No tasks.</td></tr>
              ) : (
                tasks.map((t) => (
                  <tr
                    key={t._id}
      ...
      <section>
        ...
        <p className='text-xs text-gray-500 mb-3'>Recent invoices, project milestones, and task updates (newest first).</p>
                ...
                <dl>
                  ...
                  {taskDetail.project && (
                    <>
                      ...
                      {taskDetail.project.client && (
                        <>
                          <DetailRow label='Client'>{taskDetail.project.client.clientName || '—'}</DetailRow>
                          <DetailRow label='Client contact'>{taskDetail.project.client.clientNumber || '—'}</DetailRow>
                          <DetailRow label='Client email'>{taskDetail.project.client.mailId || '—'}</DetailRow>
                          <DetailRow label='Business type'>{taskDetail.project.client.businessType || '—'}</DetailRow>
                        </>
                      )}
                    </>
                  )}
